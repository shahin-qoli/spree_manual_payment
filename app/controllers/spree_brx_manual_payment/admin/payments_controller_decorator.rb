module SpreeBrxManualPayment::Admin::PaymentsControllerDecorator
	...
	def new
        ...
        if @payment_method.source_required?
        	@payment.source = @payment_method.payment_source_class.new
        end
        @payment
	end
		...
		begin
		  if @payment_method.store_credit?
		    ...
		    if @payment.payment_method.source_required?
		    	p "JJJJJJJJJJJJJJJJJJJJJJJJJJJJ0"
		    	p source_params
		    	source_class = @payment.payment_method.payment_source_class
		    	@payment.source  = source_class.create(source_params)
		    end
		    @payment.save
		    payments = [@payment]
		  end
	...
	private
	def source_params
	  params.require(:source).permit(:from_data, :to_data, :description, :transaction_date, :manual_payment_source_type_id)
	end	
	def load_data
		...
		if @payment&.payment_method
			...
			@payment_method ||= @payment_methods.find { |payment_method| payment_method.id == ENV["MANUAL_PAYMENT_METHOD_ID"].to_i } 
		end
	end	
end

Spree::Admin::PaymentsController.prepend(SpreeBrxManualPayment::Admin::PaymentsControllerDecorator)